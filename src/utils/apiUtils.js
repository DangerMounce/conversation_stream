...
async function uploadAudioToEvaluagent(audioFile, apiKey) {
    const apiUrl = "https://api.evaluagent.com/v1";
    const url = `/quality/imported-contacts/upload-audio`
    const headers = {
        'Authorization': 'Basic ' + Buffer.from(apiKey).toString('base64')
    };
    ...
    try {
        ...
        logger.error(`There was a problem with the audio upload for `, chalk.bold.white(audioSelection), ' : ', chalk.bold.red(error.message))
        logger.error(error)
        return null
    }
}
